...
'''
2023/9/30 
運賃表が新しくなり、土気で西濃の運賃を計算、トナミを削除、久留米を削除
した時にテスト用に作ったcode
運賃を計算して、 UU_honsya, UU_toke, PH_honsya, PH_tokeを出力して
終了する。
'''
...
while True:
    try:
        uriagebi = input('~テストプログラム~ 売上日を入力してください(例: 20201204) :')
        if (
        len(uriagebi) == 8 and
...
pf = platform.system()
if pf == 'Windows':
    myfolder = r'//192.168.1.247/共有/営業課ﾌｫﾙﾀﾞ/01出荷OutPut/' + dt_now + '_oga'
else:
    myfolder = r'/mnt/public/営業課ﾌｫﾙﾀﾞ/01出荷OutPut/' + dt_now + '_oga'
